...
do
    random_uid=$(curl -s -X GET localhost:3000/user | grep '"_id"' | cut -d '"' -f 4 | sort -R | head -n 1)
    random_celeb=$(curl -s -X GET localhost:3000/celeb | grep '"_id"' | cut -d '"' -f 4 | sort -R | head -n 1)
